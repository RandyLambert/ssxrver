#ifndef SSERVER_THREADLOCAL_H
#define SSERVER_THREADLOCAL_H
...
public:
    ...
    ThreadLocal(const ThreadLocal& ) = delete ; //禁止拷贝构造
...
private:
    ...
    static void destructor(void *x) //调用这个回调函数，会去销毁数据，在调用pthread_key_create时
    {
        ...
        T_must_be_complete_type dummy; (void) dummy;//防止报错
        delete obj;//销毁
    }
